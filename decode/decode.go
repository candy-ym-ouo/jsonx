...
if u, ok := dst.Interface().(Unmarshaler); ok { if err := u.UnmarshalValue(src); err != nil { *errs = append(*errs, jerrors.WrapText(jerrors.EUser, "custom unmarshaler failed", err).WithPath(*path))
};return true;}
